...
for i in range(int(count)):
    ...
    sp = barcode_text_input("SPInventoryNumber *", f"sp{i}", MODULE_NAME)
    sp_clean = sp.strip()

    if not sp_clean or len(sp_clean) != 7 or not (sp_clean.startswith("FS") or sp_clean.startswith("SP")):
        valid = False

    inventory = barcode_text_input("InventoryNumber", f"inv{i}", MODULE_NAME)
    serial = barcode_text_input("SerialNumber", f"serial{i}", MODULE_NAME)
